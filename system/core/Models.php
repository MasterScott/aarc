...
class Models{
  ...
  private $__db_connection_settings;
  function __construct() {
    global $db,$connect;
    if(is_array($db) && is_array($connect)) {
      $this->__db_connection_settings=$this->parse_db($db,$connect);
    } else {
      $this->__db_connection_settings=$this->parse_db(array($db),array($connect));
    }
    //Initialize database connection
    if($connect) {
      if(is_array($connect)) {
        $this->load_driver($this->__db_connection_settings,$connect);
      } else {
        $this->load_driver($this->__db_connection_settings,array($connect));
      }
    }
  }
